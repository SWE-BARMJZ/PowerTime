...
public interface RepeatedTaskRepo extends JpaRepository<RepeatedTask, Long> {
    ...
    @Query("SELECT r FROM RepeatedTask r WHERE r.user = ?1 AND r.sunday = ?2 AND (r.lastRemovalDate IS NUll OR DATE(r.lastRemovalDate) <> DATE(?3))")
    Optional<List<RepeatedTask>> getAllByUserAndSundayEqualsAndLastRemovalDateNot(User user, boolean sunday, Date date);

    @Query("SELECT r FROM RepeatedTask r WHERE r.user = ?1 AND r.monday = ?2 AND (r.lastRemovalDate IS NUll OR DATE(r.lastRemovalDate) <> DATE(?3))")
    Optional<List<RepeatedTask>> getAllByUserAndMondayEqualsAndLastRemovalDateNot(User user, boolean monday, Date date);

    @Query("SELECT r FROM RepeatedTask r WHERE r.user = ?1 AND r.tuesday = ?2 AND (r.lastRemovalDate IS NUll OR DATE(r.lastRemovalDate) <> DATE(?3))")
    Optional<List<RepeatedTask>> getAllByUserAndTuesdayEqualsAndLastRemovalDateNot(User user, boolean tuesday, Date date);

    @Query("SELECT r FROM RepeatedTask r WHERE r.user = ?1 AND r.wednesday = ?2 AND (r.lastRemovalDate IS NUll OR DATE(r.lastRemovalDate) <> DATE(?3))")
    Optional<List<RepeatedTask>> getAllByUserAndWednesdayEqualsAndLastRemovalDateNot(User user, boolean wednesday, Date date);

    @Query("SELECT r FROM RepeatedTask r WHERE r.user = ?1 AND r.thursday = ?2 AND (r.lastRemovalDate IS NUll OR DATE(r.lastRemovalDate) <> DATE(?3))")
    Optional<List<RepeatedTask>> getAllByUserAndThursdayEqualsAndLastRemovalDateNot(User user, boolean thursday, Date date);

    @Query("SELECT r FROM RepeatedTask r WHERE r.user = ?1 AND r.friday = ?2 AND (r.lastRemovalDate IS NUll OR DATE(r.lastRemovalDate) <> DATE(?3))")
    Optional<List<RepeatedTask>> getAllByUserAndFridayEqualsAndLastRemovalDateNot(User user, boolean friday, Date date);

    @Query("SELECT r FROM RepeatedTask r WHERE r.user = ?1 AND r.saturday = ?2 AND (r.lastRemovalDate IS NUll OR DATE(r.lastRemovalDate) <> DATE(?3))")
